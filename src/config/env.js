...
module.exports = {
  ...
  messageLogStoreBody: parseBoolean(env.MESSAGE_LOG_STORE_BODY, true),
  messageLogBodyMaxLength: Math.max(parseNumber(env.MESSAGE_LOG_BODY_MAX_LENGTH, 1000), 0),
  retentionApiUsageDays: Math.max(parseNumber(env.RETENTION_API_USAGE_DAYS, 90), 1),
  retentionAuditDays: Math.max(parseNumber(env.RETENTION_AUDIT_DAYS, 180), 1),
  retentionWebhookDeliveryDays: Math.max(parseNumber(env.RETENTION_WEBHOOK_DELIVERY_DAYS, 90), 1),
  retentionMessageLogDays: Math.max(parseNumber(env.RETENTION_MESSAGE_LOG_DAYS, 90), 1),
