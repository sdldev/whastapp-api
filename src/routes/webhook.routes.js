...
router.get('/', asyncHandler(async (req, res) => {
  return success(res, await webhookService.listWebhooks(req.apiClient));
}));

router.get('/:webhookId', validate(webhookParamSchema), asyncHandler(async (req, res) => {
  return success(res, await webhookService.getWebhook(req.params.webhookId, req.apiClient));
}));
...
router.delete('/:webhookId', validate(webhookParamSchema), asyncHandler(async (req, res) => {
  return success(res, { deleted: await webhookService.deleteWebhook(req.params.webhookId, req.apiClient) });
}));

router.get('/:webhookId/deliveries', validate(webhookParamSchema), asyncHandler(async (req, res) => {
  return success(res, await webhookService.listDeliveries(req.params.webhookId, req.apiClient));
}));
